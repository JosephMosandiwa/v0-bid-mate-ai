...
"[project]/app/api/tenders/scraped/[id]/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request, { params }) {
    try {
        ...
        // Fetch the tender
        const { data: tender, error: tenderError } = await supabase.from("scraped_tenders").select("*").eq("id", id).single();
        if (tenderError) {
        ...
        const { data: documents, error: docsError } = await supabase.from("tender_documents").select("*").eq("tender_id", id.toString()).order("created_at", {
            ascending: false
        });
        if (docsError) {
            console.error("[v0] Error fetching documents:", docsError);
        }
        const { data: analysisData, error: analysisError } = await supabase.from("tender_analysis").select("*").eq("tender_id", id.toString()).order("created_at", {
            ascending: false
        }).limit(1).single();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0358d57d._.js.map