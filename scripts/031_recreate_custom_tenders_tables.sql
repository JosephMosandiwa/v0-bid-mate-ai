-- Recreate custom tenders tables with correct schema
-- This fixes the missing columns issue
...
DROP TABLE IF EXISTS public.user_custom_tender_responses CASCADE;
DROP TABLE IF EXISTS public.user_custom_tender_analysis CASCADE;
DROP TABLE IF EXISTS public.user_custom_tender_documents CASCADE;
DROP TABLE IF EXISTS public.user_custom_tenders CASCADE;

-- Recreate tables with correct schema
CREATE TABLE public.user_custom_tenders (
...
CREATE TABLE public.user_custom_tender_documents (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE public.user_custom_tender_analysis (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tender_id UUID NOT NULL REFERENCES public.user_custom_tenders(id) ON DELETE CASCADE,
...
CREATE TABLE public.user_custom_tender_responses (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tender_id UUID NOT NULL REFERENCES public.user_custom_tenders(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
-- Create indexes
CREATE INDEX idx_user_custom_tenders_user_id ON public.user_custom_tenders(user_id);
CREATE INDEX idx_user_custom_tenders_status ON public.user_custom_tenders(status);
CREATE INDEX idx_user_custom_tender_documents_tender_id ON public.user_custom_tender_documents(tender_id);
CREATE INDEX idx_user_custom_tender_analysis_tender_id ON public.user_custom_tender_analysis(tender_id);
CREATE INDEX idx_user_custom_tender_responses_tender_id ON public.user_custom_tender_responses(tender_id);

-- Create unique constraints
CREATE UNIQUE INDEX idx_user_custom_tender_analysis_unique ON public.user_custom_tender_analysis(tender_id);
CREATE UNIQUE INDEX idx_user_custom_tender_responses_unique ON public.user_custom_tender_responses(tender_id, user_id);
